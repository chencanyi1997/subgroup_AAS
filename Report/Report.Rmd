...
\cite{Ma2015ACP} considered this model with $p<n$, where they consider the following objective fucntion 
$$
...
$$
where $\alpha_1$ and $\alpha_2$ is the weight, and apply the ADMM algorithm, which makes the issue more severe. At this time, the ADMM algorithm has to iterate at lease ${n \choose 2} + p$ parameters sequentially where the slow convergence rate issue stands out. 
...
$$
\mathrm{BIC}(k) = \log \left[\sum_{i = 1}^n\{Y_i - \hat{\mu_i}(k) - \x_i^T\bbeta\}^2/n\right] + C_n \log n/n\{k + p\}
$$
where $C_n = c\log\{\log(n+p)\}$ with some positive constant $c$.
